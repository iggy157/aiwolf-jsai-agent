...
- Omitted fields fall back to the matching value in `<provider>:` (so you only write what you want to override).
...
```yaml
# Example: only override the model without splitting LangChain
llm:
  type: openai
  model: gpt-4o              # overrides openai.model
  separate_langchain: false
openai:
  model: gpt-4o-mini         # unused when llm.model is set (= fallback)
  temperature: 0.7
```
