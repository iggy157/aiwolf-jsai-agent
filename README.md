...
- **プロンプトブロック**: `prompts/{jp,en}/*.jinja` を `{{ block('...') }}` で再利用（日英切替は config の `lang`）、ON/OFF と 6 種のスタイルから選べる見出し付与にも対応
...
- 省略した項目は下の `<provider>:` セクションの値を継承します(全く書かなくてもOK)
...
```yaml
# 例: separate_langchain=false でも llm 直下でモデルだけ差し替え
llm:
  type: openai
  model: gpt-4o              # openai: の model を上書き
  separate_langchain: false
openai:
  model: gpt-4o-mini         # llm.model が書かれれば使われない (= フォールバック)
  temperature: 0.7
```
...
メイン config の `headings` セクションで、各ブロックの冒頭に見出しを付けるかどうか、またそのスタイルを切替えられます。プロンプト書式（order × formatting）を比較する研究用途を念頭に、6 種のスタイルを用意しています。
...
| `japanese` | `【履歴】` | `【history】` |
| `colon` | `履歴:` | `history:` |
| `xml` | `<履歴>` … `</履歴>`（閉じタグ自動付与） | `<history>` … `</history>` |
| `dash` | `- 履歴` | `- history` |
